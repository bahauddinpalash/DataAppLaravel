...
<div class="container mt-5">
    ...
    <form action="{{ route('admin_candidate_responses.store', ['recruit_lead_id' => $lead->id]) }}" method="POST">
        @csrf
        <div class="form-group mb-3">
            ...
            <select name="response_type" id="response_type" class="form-control">
                ...
                @foreach ($responseTypes as $type)
                    <option value="{{ $type }}" {{ old('response_type') == $type ? 'selected' : '' }}>{{ ucfirst($type) }}</option>
                @endforeach
            </select>
            @error('response_type')
        ...
        <a href="{{ route('admin_candidate_responses.index', ['recruit_lead_id' => $lead->id]) }}" class="btn btn-danger">Cancel</a>
    </form>
</div>
@endsection
